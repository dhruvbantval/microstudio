...
(function() {
  ...
  this.Session = class Session {
    constructor(server, socket) {
      ...
      this.register("read_component_data", (msg) => {
        return this.readComponentData(msg);
      });
      this.register("update_component_data", (msg) => {
        return this.updateComponentData(msg);
      });
    ...
    updateComponentData(data) {
      var dbData, dbPath, error, fs, objectData, objectId, parsedData, path, updatedContent;
      if (this.user == null) {
        return this.sendError("not connected");
      }
      fs = require('fs');
      ...
      try {
        ...
        if (!objectId || !objectData) {
          return this.sendError("Missing objectId or objectData");
        }
        
        // Handle both entities and objects data structures
        if (parsedData.entities && parsedData.entities[objectId]) {
          parsedData.entities[objectId] = objectData;
          console.log(`Updated entity '${objectId}' in entities`);
        } 
        else {
          return this.sendError(`entity '${objectId}' not found in db.json`);
        }
        ...
        return this.sendError(`Failed to update component data: ${error.message}`);
      }
    }
